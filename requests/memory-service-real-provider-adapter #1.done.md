...
- [x] `MemoryService` supports guarded real Mem0 retrieval path.
- [x] Fallback behavior remains deterministic when provider unavailable/fails.
- [x] Contract output shape unchanged (`ContextCandidate` and recall envelope remain stable).
- [x] New adapter tests pass without external network dependency.
- [x] No router/fallback policy regressions introduced.
...
- [ ] Manual validation steps executed (deferred to runtime)
- [x] Execution report generated (`.done.md`)
