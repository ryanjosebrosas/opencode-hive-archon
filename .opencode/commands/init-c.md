---
description: Initialize AGENTS.md for a new or existing project
agent: build
---
...
You will generate a **modular** structure — NOT a flat AGENTS.md file. The output is:

1. **A slim `AGENTS.md`** (30-60 lines) containing ONLY `@sections/` references
...
## AGENTS.md Structure (What You Generate)

The generated AGENTS.md must look like this:
...
**CRITICAL**: AGENTS.md contains ONLY `@sections/` references, NEVER inline content. Each `---` separator is required between sections.

**Line Count Constraints**:
- AGENTS.md: 30-60 lines (only @references)
...
4. **Generate slim AGENTS.md** with only `@sections/` references
5. **Suggest reference/ guides** for task-specific patterns
...
3. Write the slim `AGENTS.md` with `@sections/` references
