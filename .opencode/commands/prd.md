...
The detailed build blueprint. Takes the vision from `/mvp` and expands it into a full product specification: architecture, user stories, tech stack, API spec, phases, risks.

## Pipeline Position

```
/mvp (big idea) → /prd (full details) → /decompose (spec list) → /build next → /ship
```

This is Step 2. Reads `mvp.md` as input. Output feeds `/decompose`.

## Output File

Write the PRD to: `$ARGUMENTS` (default: `PRD.md`)

Path safety and intent disambiguation (mandatory):
- If `$ARGUMENTS` points to an existing file, do NOT overwrite immediately.
- First ask whether that path is:
  1) source input/example to analyze, or
  2) output destination to overwrite.
- If ambiguous, default to creating a new sibling file with `-prd.md` suffix.
...
### 0. Mandatory Spec Lock (before drafting)
- Ask and lock these items before generating PRD content:
  1. Implementation mode: docs-only, code implementation, or both
  2. Target repository/path
  3. Preferred tech stack/framework (or confirm "follow existing stack")
  4. Maturity target: alpha, MVP, or production
  5. Required output depth: lightweight PRD or full deep PRD
- If user already provided this, restate it as a "Spec Lock" summary and request explicit confirmation.

### 0.5 Mandatory Preview + Approval Gate
- Generate a concise PRD preview first:
  - problem framing
  - scope in/out
  - architecture direction
  - assumptions
  - research plan
- Ask: "Approve this PRD direction before I write the final file?"
- Only write the full PRD after explicit approval.
...
- Fill in assumptions only when needed, and label each as `Assumption` with rationale
- Maintain consistency across sections
- Ensure technical feasibility

Evidence quality requirement (mandatory):
- Include concrete codebase evidence where relevant (file paths and line refs).
- Include external docs references for major architecture choices.
- Do not produce generic architecture claims without evidence.
...
- ✅ Spec Lock items are present and confirmed
- ✅ Preview was approved before file write
...
- Never skip the Spec Lock and preview approval gate
