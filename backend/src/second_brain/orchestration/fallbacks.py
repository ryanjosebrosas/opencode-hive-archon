...
class FallbackEmitter:
    ...
    def emit_channel_mismatch(
        candidates: list[ContextCandidate], expected_channel: str, provider: str = "unknown"
    ) -> tuple[ContextPacket, NextAction]:
    ...
    def emit_rerank_bypassed(
        candidates: list[ContextCandidate], provider: str = "mem0"
    ) -> tuple[ContextPacket, NextAction]:
    ...
    @staticmethod
    def emit_success(
        candidates: list[ContextCandidate], provider: str = "unknown", rerank_applied: bool = False
    ) -> tuple[ContextPacket, NextAction]:
...
def determine_branch(
    ...
    if top_confidence < threshold:
        return FallbackEmitter.emit_low_confidence(candidates, top_confidence, threshold, provider)
