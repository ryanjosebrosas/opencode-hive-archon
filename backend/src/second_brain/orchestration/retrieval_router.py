...
class RouteDecision:
    ...
    def select_route(
        ...
        if mode == "conversation":
            ...
            if "mem0" in available_providers and _is_provider_eligible(
                "mem0", available_providers, provider_status
            ):
                status = provider_status.get("mem0", ProviderStatus.AVAILABLE)
                if status == ProviderStatus.AVAILABLE:
                    return "mem0", {"skip_external_rerank": True}  # Mem0 policy
            if "supabase" in available_providers and _is_provider_eligible(
                "supabase", available_providers, provider_status
            ):
            ...
            for provider in ["mem0", "supabase", "graphiti"]:
                if provider in available_providers and _is_provider_eligible(
                    provider, available_providers, provider_status
                ):
